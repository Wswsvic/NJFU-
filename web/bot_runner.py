...
from src.bot.operating_logger import OperatingLogger
...
def _get_logger() -> OperatingLogger:
    global _op_logger
    if _op_logger is None:
        _op_logger = OperatingLogger(_DEBUG_DIR)
    return _op_logger
...
def execute_reservation(plan: dict) -> tuple:
    ...
    logger = _get_logger()
    logger.start_session(plan["id"], user["username"], room_id, seat_name)
